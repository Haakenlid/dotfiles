...
def main():
    ...
    reged = subprocess.Popen(
        ["reged", "-x", options.system, '\\', options.key, out],
        stdout=sys.stderr)
    reged.wait()
    if reged.returncode:
    ...
    for section in dump:
        ...
        if len(path) == 3:
            path[1] = ':'.join(
                [path[1][i:i + 2] for i in range(0, len(path[1]), 2)]).upper()
            path[2] = ':'.join(
                [path[2][i:i + 2] for i in range(0, len(path[2]), 2)]).upper()
            print("Dumping {}/{}...".format(path[1], path[2]))
            config = ConfigParser()
            config.optionxform = str

            # See if device has been paired in Linux before
            existing_template = '/var/lib/bluetooth/{}/{}/info'.format(
                path[1], path[2])
            ...
            def read_reg(key, expected_type):
                ...
                def read_reg_actual(key, expected_type):
                    actual_type, content = dump[section]['"{}"'.format(
                        key)].split(':', 1)
                ...
                if options.verbose:
                    print("{} of type {}: {}".format(key, expected_type,
                                                     result))
                return result
            ...
            if options.attributes:
                shutil.copyfile(options.attributes,
                                os.path.join(output_dir, 'attributes'))
